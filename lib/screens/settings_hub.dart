...
Future<void> _settingsSetProgressivePush(bool value) async {
	...
	if (!value) {
		await NotificationService().cancelNotification(kCurrentLessonNotificationId);
	} else {
...
Future<void> _settingsSetDailyBriefingPush(bool value) async {
	...
	if (!value) {
		await NotificationService().cancelNotification(kDailyBriefingNotificationId);
	} else {
		updateUntisData().catchError((_) {});
	}
}
...
Future<void> _settingsSyncFromPrefs() async {
	...
	appLocaleNotifier.value = prefs.getString('appLocale') ?? appLocaleNotifier.value;
	themeModeNotifier.value = ThemeMode.values[(prefs.getInt('themeMode') ?? 0).clamp(0, 2)];
	showCancelledNotifier.value = prefs.getBool('showCancelled') ?? showCancelledNotifier.value;
	...
	blurEnabledNotifier.value = prefs.getBool('blurEnabled') ?? blurEnabledNotifier.value;
	progressivePushNotifier.value = prefs.getBool('progressivePush') ?? progressivePushNotifier.value;
