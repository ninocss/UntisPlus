...
final _themeModePreference = PreferenceBinding<ThemeMode>(
  ...
  decoder: (raw) => ThemeMode.values[
      ((raw is int ? raw : 0).clamp(0, ThemeMode.values.length - 1)).toInt()
  ],
...
final _themeBlurPreferencesPreference =
    PreferenceBinding<Map<String, bool>>(
      key: 'themeBlurPreferences',
      defaultValue: Map<String, bool>.from(
        themeBlurPreferencesNotifier.value,
      ),
      notifier: themeBlurPreferencesNotifier,
      decoder: (raw) => AppThemeIdX.normalizeBlurPreferences(
        raw is String ? decodeJsonMap(raw) : raw is Map ? raw : null,
        defaultThemeBlur:
...
Future<void> _settingsSetDailyBriefingPush(bool value) async {
  ...
  if (!value) {
    await NotificationService().cancelNotification(NotificationIds.dailyBriefing);
  } else {
    updateUntisData().catchError((_) => false);
  }
}
